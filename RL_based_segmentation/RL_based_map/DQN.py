...
def get_player(directory=None, files_list=None, landmark_ids=None, viz=False,
               task="eval", file_type="brain", saveGif=True, saveVideo=True,
               multiscale=True, history_length=20, agents=1, logger=None, w=0):
    env = MedicalPlayer(
        directory=directory,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load', help='Path to the model to load',
                        default='runs/Mar01_15-00-55_sepideh/latest_dqn_9579.pt')
    parser.add_argument(
        '--task',
    ...
    parser.add_argument(
        ...
        choices=['brain', 'cardiac', 'fetal'], default='brain')
    parser.add_argument(
        ...
        help="""Filepath to the text file that contains list of images.
                ...
                ['images', 'landmarks', 'distance maps', 'start points']""",
        # default=['data/filenames/image_test_files_f1.txt',
        #          'data/filenames/start_points_test_f1.txt',
        #          'data/filenames/distance_map_test_f1.txt',
        #          'data/filenames/landmark_test_files_f1.txt']
        default=['data/filenames/image_files_f1.txt',
                 'data/filenames/start_points_f1.txt',
                 'data/filenames/distance_map_f1.txt',
                 'data/filenames/landmark_files_f1.txt']
              )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="Network3d", choices=['CommNet', 'Network3d'], type=str)
    parser.add_argument(
    ...
    parser.add_argument(
        '--w',
        help='value of reward or punishment for distance of centerline',
        default=1, type=int)
    parser.add_argument(
        '--seed',
    ...
    parser.add_argument(
        ...
        help='Starting position of the agents during rollout. Randomised if not specified.', default=[0.5, 0.5, 0.5])
    ...
    if args.task == 'play':
        error_message = f"""Wrong input files {len(args.files)} for {args.task}
                            task - should be 2 \'images.txt\' """
        assert len(args.files) == 2, (error_message)
    else:
        error_message = f"""Wrong input files {len(args.files)} for
                            {args.task} task - should be 3 [\'images.txt\',
                            \'landmarks.txt\'] """
        assert len(args.files) == 4, (error_message)
    ...
    if args.task != 'train':
        ...
        environment = get_player(files_list=args.files, landmark_ids=args.landmarks, viz=args.viz, task=args.task,
                                 file_type=args.file_type, saveGif=args.saveGif, saveVideo=args.saveVideo,
        ...
        environment = get_player(files_list=args.files, landmark_ids=args.landmarks, viz=args.viz, task='train',
                                 file_type=args.file_type, multiscale=args.multiscale, agents=agents, logger=logger)
        eval_env = None
        if args.val_files is not None:
            eval_env = get_player(files_list=args.val_files, landmark_ids=args.landmarks, task='eval',
                                  file_type=args.file_type, agents=agents, logger=logger)
