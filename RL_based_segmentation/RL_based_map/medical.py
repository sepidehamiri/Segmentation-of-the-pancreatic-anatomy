...
class MedicalPlayer(gym.Env):
    ...
    def __init__(self, directory=None, viz=False, task=False, files_list=None,
                 ...
                 oscillations_allowed=4, fixed_spawn=None, logger=None, w=1):
        """
    ...
    def new_random_game(self, fixed_spawn=None):
        ...
        # sample a new image
        self._image, self._target_loc, self.filepath, self._start_loc, self.spacing, self._map = next(
            self.sampled_files)
        self.filename = [
            os.path.basename(
        ...
        if fixed_spawn is None:
            ...
            x = np.array([int(self._start_loc[0][0][0])])
            y = np.array([int(self._start_loc[0][0][1])])
            z = np.array([int(self._start_loc[0][0][2])])
            # x = np.random.randint(
            #         skip_thickness[0],
    ...
    def calcDistance(self, points1, points2, spacing=(1, 1, 1)):
        ...
        # print('point 1', points1)
        # print('point 2', points2)
        spacing = np.array(spacing)
        points1 = spacing * np.array(points1)
        points2 = spacing * np.array(points2)
        return np.linalg.norm(points1 - points2)

    def calcPointLineDistance(self, point, start_point, end_point):
        """ calculate the distance between point and line in mm"""
        # normalized tangent vector
        d = np.divide(end_point - start_point, np.linalg.norm(end_point - start_point))

        # signed parallel distance components
        s = np.dot(start_point - point, d)
        t = np.dot(point - point, d)

        # clamped parallel distance
        h = np.maximum.reduce([s, t, 0])

        # perpendicular distance component
        c = np.cross(point - start_point, d)
    ...
    def _calc_reward(self, current_loc, next_loc, agent):
        ...
        next_dist = self.calcDistance(next_loc, self._target_loc[agent],
                                      self.spacing)
        line_dist = self.calcPointLineDistance(np.asarray(current_loc), np.asarray(self._start_location[agent]),
                                               np.asarray(self._target_loc[agent]))
        curr_dist_center = self._map[0].data[current_loc]
        next_dist_center = self._map[0].data[next_loc]
        # if curr_dist_center - next_dist_center >= 0:
        #     r = 1
        # else:
        #     r = -1
        return (curr_dist - next_dist) + ((curr_dist_center - next_dist_center)/255)
        ...
        if self.saveGif:
            ...
            if all(self.terminal):
                ...
                self.viewer.saveGif(gifname, arr=self.gif_buffer,
                                    duration=self.viz)
        if self.saveVideo:
            dirname = 'tmp_video_pancreas' + self.filename[0]
            if self.cnt <= 1:
