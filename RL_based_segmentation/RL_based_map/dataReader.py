...
class filesListBrainMRLandmark(object):
    ...
    def __init__(self, files_list=None, returnLandmarks=True, agents=1):
        ...
        self.distance_map = [line.split('\n')[0]
                            for line in open(files_list[2])]
        self.start_points = [line.split('\n')[0]
                            for line in open(files_list[3])]
    ...
    def sample_circular(self, landmark_ids, shuffle=False):
        ...
        while True:
            for idx in indexes:
                ...
                sitk_dist, distance_map = NiftiImage().decode(self.distance_map[idx])

                if self.returnLandmarks:
                    # transform landmarks to image space if they are in
                    # physical space
                    landmark_file = self.landmark_files[idx]
                    start_point_file = self.start_points[idx]
                    all_landmarks = getLandmarksFromTXTFile(landmark_file)
                    all_start = getLandmarksFromTXTFile(start_point_file)
                ...
                maps = [distance_map] * self.agents
                starts = [all_start] * self.agents
                yield (images, landmarks, image_filenames, starts,
                       sitk_image.GetSpacing(), maps)
###############################################################################
    ...
    def sample_circular(self, landmark_ids, shuffle=False):
        ...
        while True:
            for idx in indexes:
                ...
                if self.returnLandmarks:
                    landmark_file = self.landmark_files[idx]
                    start_point_file = self.start_point_files[idx]
