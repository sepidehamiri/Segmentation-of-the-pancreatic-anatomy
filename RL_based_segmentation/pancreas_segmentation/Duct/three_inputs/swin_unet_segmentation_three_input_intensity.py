...
from monai.transforms import (
    ...
    Spacingd,
    RandRotate90d, EnsureChannelFirstD,
...
directory = "/home/navid/Desktop/Papers/paper2/pancreas_segmentaion/Duct/three_inputs/model_log/swin_unetr"
...
train_transforms = Compose(
    [
        ...
        #     b_min=0.0, b_max=1.0, clip=False,
        # ),
        CropForegroundd(keys=keys, source_key="image_ct", allow_smaller=False),
...
intensity_dir = '/home/navid/Desktop/Papers/paper2/pancreas_segmentaion/duct_intensity_registered_masks'
images_sg = sorted(glob.glob(os.path.join(data_dir, '*', "mask*")))
images_pt = sorted(glob.glob(os.path.join(intensity_dir, "final_mask_*")))
...
num_folds = 4
fold_size = len(data_dict) // num_folds

for f in range(1, num_folds + 1):
    start_val = (f - 1) * fold_size
    end_val = start_val + fold_size
    if f == num_folds:
        end_val = len(data_dict)  # Ensure the last fold includes the remainder

    train_files = data_dict[:start_val] + data_dict[end_val:]
    val_files = data_dict[start_val:end_val]
    # Fold 1
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (
                ...
                if dice_val > dice_val_best:
                    ...
                    torch.save(
                        model.state_dict(), os.path.join(root_dir, f"best_metric_model_3input_f{f}.pth")
                    )
                    print(
    ...
    model.load_state_dict(torch.load(os.path.join(root_dir, f"best_metric_model_3input_f{f}.pth")))
