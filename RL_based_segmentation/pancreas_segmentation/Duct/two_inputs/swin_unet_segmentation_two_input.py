...
directory = "model_log/swin_unetr"
...
val_transforms = Compose(
    [
        ...
        EnsureTyped(keys=["image_ct", "image_sg"], device=device),
    ]
)
data_dir = '/home/navid/Documents/cropped'
images_sg = sorted(glob.glob(os.path.join(data_dir, 'mask', "case*")))
images_ct = sorted(glob.glob(os.path.join(data_dir, 'ct', "case*")))
...
#                 torch.save(
#                     model.state_dict(), os.path.join(root_dir, "best_metric_model_f4.pth")
#                 )
#                 print(
...
def time_test(epoch_iterator_test, model):
    ...
    with torch.no_grad():
        for step, batch in enumerate(epoch_iterator_test):
            ...
            nib.save(img, f'siemens_duct_swin_output/ct_{step}.nii.gz')
            nib.save(seg, f'siemens_duct_swin_output/sg_{step}.nii.gz')
            nib.save(pred, f'siemens_duct_swin_output/pred_{step}.nii.gz')
...
#     }
    case_num = 4
model.load_state_dict(torch.load(os.path.join(root_dir, "fold2_swin_2inputs_duct")), strict=False)
