...
for f in range(1, 5):
    ...
    #                 torch.save(
    #                     model.state_dict(), os.path.join(root_dir, f"best_metric_unet_model_f{f}.pth")
    #                 )
    #                 print(
    ...
    #     )
    # model.load_state_dict(torch.load(os.path.join(root_dir, f"best_metric_unet_model_f{f}.pth")))
...
def time_test(epoch_iterator_test, model):
    ...
    with torch.no_grad():
        for step, batch in enumerate(epoch_iterator_test):
            ...
            nib.save(img, f'unet_output/ct_{step}_{f}.nii.gz')
            nib.save(seg, f'unet_output/sg_{step}_{f}.nii.gz')
            nib.save(pred, f'unet_output/pred_{step}_{f}.nii.gz')
