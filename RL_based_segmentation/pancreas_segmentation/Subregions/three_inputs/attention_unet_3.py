...
data_dir = '/science/image/cluster-homedirs/wgz342/subregion/subregion_dataset'
...
for f in range(2, 7):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (
                ...
                if dice_val > dice_val_best:
                    ...
                    torch.save(
                        model.state_dict(), os.path.join(root_dir, f"best_metric_model_attention_f{f}.pth")
                    )
                    print(
    ...
    model.load_state_dict(torch.load(os.path.join(root_dir, f"best_metric_model_attention_f{f}.pth")))
...
def test_time(epoch_iterator_test, model):
    model.eval()
    with torch.no_grad():
...
for f in range(1, 7):
    ...
    test_time(epoch_iterator_test, model)
