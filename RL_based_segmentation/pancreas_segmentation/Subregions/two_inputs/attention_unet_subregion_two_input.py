...
from monai.transforms import (
    ...
    RandShiftIntensityd, Orientationd,
    RandRotate90d, AddChanneld, SaveImage,
...
directory = "model_log/attention_unet_subregions"
...
data_dict = [
    {"image_ct": image_name_ct, "image_sg": image_name_sg}
    for image_name_ct, image_name_sg in zip(images_ct, images_sg)
]
...
for f in range(1, 7):
    ...
    #             val_inputs, val_labels = (batch["image_ct"].cuda(), batch["image_sg"].cuda())
    ...
    #         step += 1
    #         x, y = (batch["image_ct"].cuda(), batch["image_sg"].cuda())
    #         with torch.cuda.amp.autocast():
    #             logit_map = model(x)
    ...
    #                 torch.save(
    #                     model.state_dict(), os.path.join(root_dir, f"best_metric_subregions_attention_unet_model_f{f}.pth")
    #                 )
    #                 print(
    ...
    #     )
    # model.load_state_dict(torch.load(os.path.join(root_dir, f"best_metric_subregions_attention_unet_model_f{f}.pth")))
    #
    # print(
...
def time_test(epoch_iterator_test, model):
    ...
    with torch.no_grad():
        for step, batch in enumerate(epoch_iterator_test):
            test_inputs, test_labels = (batch["image_ct"].cuda(), batch["image_sg"].cuda())
            with torch.cuda.amp.autocast():
            ...
            # fillholes = [FillHoles(applied_labels=[0, 1, 2, 3, 4], connectivity=3)(i) for i in test_output_convert]
            # largest = [KeepLargestConnectedComponent(applied_labels=[0, 1, 2, 3, 4], is_onehot=True, connectivity=3)(i)
            #            for i in fillholes]
            # test_output_convert = [
            #     post_transforms(test_pred_tensor) for test_pred_tensor in largest
            # ]
            pred_np = test_output_convert[0].squeeze(axis=0).cpu().detach().numpy()
            # pred_np_arg = pred_np.reshape(1, 0, 2)
            ...
            pred = nib.Nifti1Image(pred_np, affine=np.eye(4))

            nib.save(img, f'attention_pred_regions/ct_{step}_{f}.nii.gz')
            nib.save(seg, f'attention_pred_regions/sg_{step}_{f}.nii.gz')
            nib.save(pred, f'attention_pred_regions/pred_{step}_{f}.nii.gz')
