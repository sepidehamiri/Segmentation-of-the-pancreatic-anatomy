...
directory = "model_log/swin_unet_subregions"
...
def time_test(epoch_iterator_test, model):
    ...
    with torch.no_grad():
        for step, batch in enumerate(epoch_iterator_test):
            ...
            largest = [KeepLargestConnectedComponent(applied_labels=[1, 2, 3, 4], is_onehot=True, connectivity=3)(i)
                       for i in fillholes]
            test_output_convert = [
                post_pred(test_pred_tensor) for test_pred_tensor in largest
            ]
            ...
            nib.save(img, f'swin_pred_regions/ct_{step}_{f}.nii.gz')
            nib.save(seg, f'swin_pred_regions/sg_{step}_{f}.nii.gz')
            nib.save(pred, f'swin_pred_regions/pred_{step}_{f}.nii.gz')

    #         dice_metric(y_pred=largest, y=test_labels_convert)
