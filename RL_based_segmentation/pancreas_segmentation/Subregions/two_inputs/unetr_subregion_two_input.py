...
print_config()
directory = "model_log/unetr_subregions_two_input"
...
data_dir = '/home/navid/Desktop/Papers/paper2/pancreas_segmentaion/subregion_dataset'
images_sg = sorted(glob.glob(os.path.join(data_dir, '*', "mask*")))
images_ct = sorted(glob.glob(os.path.join(data_dir, '*', "image*")))

data_dict = [
    {"image": image_name_ct, "label": image_name_sg}
    for image_name_ct, image_name_sg in zip(images_ct, images_sg)
]

train_files = data_dict[:57]
val_files = data_dict[57:70]
...
case_num = 0
img_name = os.path.split(val_ds[case_num]['image_meta_dict']["filename_or_obj"])[0][-2:]
