...
directory = "model_log/unet_subregions"
...
def time_test(epoch_iterator_test, model):
    ...
    with torch.no_grad():
        for step, batch in enumerate(epoch_iterator_test):
            ...
            nib.save(img, f'unet_pred_regions/ct_{step}_{f}.nii.gz')
            nib.save(seg, f'unet_pred_regions/sg_{step}_{f}.nii.gz')
            nib.save(pred, f'unet_pred_regions/pred_{step}_{f}.nii.gz')
